...
class TaskHandler:
    ...
    def _get_javscript_instrumentation_subtask(self):
        ...
        return make_request(
            Path.JAVSCRIPT_INSTRUMENTATION_URL, params=params, session=self.__session
        )
    ...
    def _get_user_flow_token(
        ...
        return make_request(
            Path.TASK_URL, method="POST", json=payload, session=self.__session
        )
    ...
    def login(self, username, password):
        ...
        try:
            task_flow_mapper = self._create_task_mapper(
                username, password, verification_input_data
            )
